...
Over the **Jan 01, 2019 – Jan 01, 2026** backtest window, the strategy achieved an **absolute cumulative return of +3,000.00%**, capturing significant absolute upside by dynamically layering the leverage corridor on top of TSLA's own structural directional runs over that period.

This figure is reported as an **absolute return**, not as outperformance (alpha) relative to a buy-and-hold baseline. The distinction matters: TSLA's own price action over the same window accounts for the large majority of this number, and the strategy's contribution should be assessed separately via risk-adjusted metrics rather than the headline return in isolation.

| Metric | Strategy | Buy & Hold TSLA |
|---|---|---|
| Cumulative return | +3,000.00% | +2,450.00% |
| Max drawdown | -76.74% | -73.63% |
| Calmar ratio | 39.09 | 33.27 |
| Backtest window | Jan 01, 2019 – Jan 01, 2026 | Jan 01, 2019 – Jan 01, 2026 |

## 7. Backtest Disclosures & Risk Caveats

- **These figures reflect historical simulation, not live execution.** No trades in this backtest were placed against live, executable prices, and no slippage or financing-cost model beyond what is described in Section 4 has been applied.
- **The strategy's nominal returns are heavily dominated by the underlying asset's own high-beta tailwinds**, not by the alpha modifiers in isolation. A strategy that is permanently near-fully long a high-beta asset during a strong directional run will post a large absolute return largely independent of any signal layer applied on top of it.
- **The alpha modifiers are designed to optimise risk-adjusted efficiency** — principally the Calmar ratio (return relative to maximum drawdown) — **rather than to serve as a market-neutral source of edge.** They are a refinement to how much exposure is held within the leverage corridor at any given time, not a claim that the underlying directional bet itself is uncorrelated with TSLA.
- **Backtested performance does not imply forward-looking performance.** Historical results, however large, are not a guarantee of future returns, and no assurance is made that the relationships exploited by the sentiment and volatility modifiers will persist going forward.
